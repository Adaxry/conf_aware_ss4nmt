...
def read_files(names):
    inputs = [[] for _ in range(len(names) + 1)]
    files = [tf.gfile.GFile(names[0]), tf.gfile.GFile(names[1]), tf.gfile.GFile(names[1])] # duplicate trg file

    for lines in zip(*files):
        lines = [line.strip() for line in lines]
        lines[-1] = " ".join([np.random.choice(lines[-1].split()) for _ in lines[-1].split()])  # the last is rand_trg file, 
    ...
    with tf.device("/cpu:0"):
        ...
        dataset = dataset.padded_batch(
            params.eval_batch_size,
            {
                ...
                "target_length": [],
                "rand_target": [tf.Dimension(None)],
            },
            {
                "source": params.pad,
                "source_length": 0,
                "target": params.pad,
                "rand_target": params.pad,
                "target_length": 0
            }
        )
