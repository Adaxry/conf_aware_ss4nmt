...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        def parse_fn(line_src, line_trg):
            tokens_src = [s.encode() for s in line_src.strip().split()]
            tokens_trg = [t.encode() for t in line_trg.strip().split()]
            rand_tokens_trg = [t.encode() for t in line_trg.strip().split()]
            if np.random.random() < params.happen_prob:
                if params.random_trg_from_vocab:
                    rand_tokens_trg = [np.random.choice(params.vocabulary["target"]) if np.random.random() < params.replace_prob else token for token in tokens_trg]
                else:
                    rand_tokens_trg = [np.random.choice(tokens_trg) if np.random.random() < params.replace_prob else token for token in tokens_trg]
            return tokens_src, tokens_trg, rand_tokens_trg


        def generator_fn(src_file, trg_file):
            with Path(src_file).open('r', encoding="utf-8") as src_f, Path(trg_file).open('r', encoding="utf-8") as trg_f:
                for line_src, line_trg in zip(src_f.readlines(), trg_f.readlines()):
                    yield parse_fn(line_src, line_trg)
        ...
        '''  
        # it seams only use the bellow 7 lines, the next map func will have no error
        dataset = dataset.map(
            lambda src, tgt, rand_tgt: (
                tf.concat([src, [tf.constant(params.eos)]], axis=0),
                tf.concat([tgt, [tf.constant(params.eos)]], axis=0),
                tf.concat([rand_tgt, [tf.constant(params.eos)]], axis=0)
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        features["rand_target"] = tgt_table.lookup(features["rand_target"])
        ...
        features["rand_target"] = tf.to_int32(features["rand_target"])
